...
| F2.2 Schema delta | `prisma/schema.prisma` | Expected: none (CheckResult/Contact/Outreach/SourcingRun exist). Document `halted` as a legal Outreach.status value in the field comment + `src/lib/db/enums.ts` (note: PLAN references this file but Phase 0 never created it — create it here as the status-vocabulary module both P2 and P4 import). |

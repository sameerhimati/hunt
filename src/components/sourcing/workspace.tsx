...
export function SourcingWorkspace({
  ...
  return (
    <div className="flex flex-col gap-3.5">
      {degraded ? (
        <DegradedBanner
          title="Sourcing needs a job-search API key"
          body={
            <>
              Add <span className="text-foreground">JSearch</span> or{' '}
              <span className="text-foreground">Adzuna</span> in Settings to search the boards. You
              can still add jobs any time by pasting a URL.
            </>
          }
          settingsHref="/settings#jsearch"
        />
      ) : null}
